...
impl SearchEngine {
  ...
  pub fn new() -> SearchEngine {
    SearchEngine {
      ...
      normalized_cache: RefCell::new(LruCache::new(
        NonZeroUsize::new(250).unwrap(),
      )),
    }
  }
  ...
  pub fn search(&self, query: &str, max_results: usize) -> String {
    ...
    // Filter out any results with NaN scores to prevent JSON serialization
    // issues
  ...
  fn safe_serialize_to_json<T: serde::Serialize>(
    &self,
    data: &T,
    fallback: &str,
  ) -> String {
...
impl SearchEngine {
  ...
  fn execute_search(&self, query_terms: &[&str]) -> Vec<SearchResult> {
    ...
    let mut post_matches: HashMap<usize, (bool, bool, Vec<String>)> =
      HashMap::new();
      ...
      if let Some(entries) = self.word_index.get(term) {
        for &(post_idx, weight) in entries {
          *post_scores.entry(post_idx).or_insert(0.0) += weight;
          ...
          // Track match types
          if let Some(post) = self.posts.get(post_idx) {
            let entry = post_matches.entry(post_idx).or_insert((
              false,
              false,
              Vec::new(),
            ));
